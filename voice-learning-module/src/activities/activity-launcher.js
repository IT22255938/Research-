...
export class ActivityLauncher {
  constructor(activity, studentId, currentLevel = 1, voiceManager = null) {
    ...
    this.autoProgressEnabled = true; // Enable auto-progression to next question
    this.autoProgressDelay = 2000; // Delay before moving to next question (ms)
    this.onQuestionChanged = null; // Callback when question changes
    this.onActivityCompleted = null; // Callback when activity ends
  }
  ...
  async recordVoiceResponse(question, voiceResult) {
    try {
      ...
      return {
        ...
        feedback: isCorrect ? question.feedback.correct : question.feedback.incorrect,
        confidence: confidence,
        autoProgressNext: isCorrect // Flag for auto-progression
      };
      };
    } catch (error) {
  ...
  autoProgressToNextQuestion() {
    if (!this.autoProgressEnabled) {
      return; // Auto-progression disabled
    }

    console.log(`⏱️ Auto-progressing in ${this.autoProgressDelay}ms...`);
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        const hasMoreQuestions = this.currentQuestionIndex < (this.activity.questionBank?.length || 0);
        
        if (hasMoreQuestions && this.onQuestionChanged) {
          console.log(`✅ Moving to question ${this.currentQuestionIndex + 1}`);
          this.onQuestionChanged(this.getNextQuestion());
        } else if (!hasMoreQuestions && this.onActivityCompleted) {
          console.log(`🏁 Activity completed!`);
          this.onActivityCompleted(this.getSummary());
        }
  ...
  /**
   * Set auto-progression settings
   */
  setAutoProgression(enabled = true, delayMs = 2000) {
    this.autoProgressEnabled = enabled;
    this.autoProgressDelay = delayMs;
    console.log(`🔄 Auto-progression: ${enabled ? 'ENABLED' : 'DISABLED'} (${delayMs}ms delay)`);
  }

  /**
   * Set callbacks for progression
   */
  setProgressionCallbacks(onQuestionChanged, onActivityCompleted) {
